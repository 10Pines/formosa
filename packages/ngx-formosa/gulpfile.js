...
const config = {
  libraryName: 'ngx-formosa',
  unscopedLibraryName: 'ngx-formosa',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('rollup-bundle', (cb) => {
  ...
  .then(() => {
    ...
    const globals = {
      ...
      'formosa': _.camelCase('formosa'.replace('/','.'))
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /formosa/ --deploy-url /formosa/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-formosa-demo:server`, { cwd: `${config.demoDir}` }))
